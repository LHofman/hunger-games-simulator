...
class Sponsors(EventRule):
    ...
    def replace_text_terms(
        ...
        if text.find('(Sponsor') == -1: return text_and_terms
        ...
        is_fixed_sponsor = (
            game_state['options'].get('one_sponsor_per_tribute', False)
        )
        if (
        ...
        if text.find('(Sponsor::opposing)') == -1:
            ...
            current_tribute_sponsor = (
                sponsors[game_state['current_tribute']['index'] - 1]
            )
            return {
                **text_and_terms,
